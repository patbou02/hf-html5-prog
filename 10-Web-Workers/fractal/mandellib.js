...
function drawRow(workerResults) {
  let values = workerResults.values;	// The values array the worker sends back
  let pixelData = rowData.data;		// The actual pixels in the ImageData obj
  // The pixelData is a *reference* to the
  // 	rowData.data! so changing pixelData
  // 	changes the rowData.data!!!
  for (let i = 0; i < rowData.width; i++) {  // for each pixel in the row
    let red = i * 4;
    let green = i * 4 + 1;
    let blue = i * 4 + 2;
    let alpha = i * 4 + 3;
